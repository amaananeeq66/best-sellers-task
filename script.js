...
function buildStars(rating) {
  ...
  let html = "";
  for (let i = 1; i <= 5; i++) {
    const fill = i <= rating ? "fill-gray-900" : "fill-gray-200";
    html += `<svg class="w-2.5 h-2.5 ${fill}" viewBox="0 0 20 20">${path}</svg>`;
  }
  return html;
}
function buildDesktopCard(p) {
  const saveBadge = p.saveBadge
    ? `<span class="absolute top-2 right-2 z-10 text-[8px] font-bold uppercase tracking-wide bg-lime-300 text-gray-900 px-2 py-0.5 rounded-full">${p.saveBadge}</span>`
    : "";
  return `
                <div class="group flex-none cursor-pointer" style="width:calc(22.2% - 10px);min-width:160px;scroll-snap-align:start;">
                    ...
                    ${saveBadge}
                    <img class="absolute inset-0 w-full h-full object-cover transition-opacity duration-300 group-hover:opacity-0"
                        src="${p.imgPrimary}" alt="${p.name}"/>
                    <img class="absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                        src="${p.imgHover}" alt="${p.name} hover"/>
                    </div>
...
products.forEach((p) => {
  track.insertAdjacentHTML("beforeend", buildDesktopCard(p));
  mobileGrid.insertAdjacentHTML("beforeend", buildMobileCard(p, true));
});
